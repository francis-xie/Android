...
public class Client {
    ...
    public AlipayUserCertifyOpenInitializeResponse init(String outerOrderNo, String bizCode, IdentityParam identityParam, MerchantConfig merchantConfig) throws Exception {
        HttpModel.validateParams(identityParam, "identityParam");
        HttpModel.validateParams(merchantConfig, "merchantConfig");
        ...
        while (Http.allowRetry((java.util.Map<String, Object>) runtime_.get("retry"), _retryTimes, _now)) {
            ...
            try {
                ...
                java.util.Map<String, String> systemParams = HttpConverter.buildMap(
                    new HttpPair("method", "alipay.user.certify.open.initialize"),
                ...
                java.util.Map<String, Object> bizParams = HttpConverter.buildMap(
                    new HttpPair("outer_order_no", outerOrderNo),
                    new HttpPair("biz_code", bizCode),
                    new HttpPair("identity_param", identityParam),
                    new HttpPair("merchant_config", merchantConfig)
                );
                ...
                if (_kernel.isCertMode()) {
                    if (_kernel.verify(respMap, _kernel.extractAlipayPublicKey(_kernel.getAlipayCertSN(respMap)))) {
                        return HttpModel.toModel(_kernel.toRespModel(respMap), new AlipayUserCertifyOpenInitializeResponse());
                    }
    ...
    public AlipayUserCertifyOpenQueryResponse query(String certifyId) throws Exception {
        ...
        while (Http.allowRetry((java.util.Map<String, Object>) runtime_.get("retry"), _retryTimes, _now)) {
            ...
            try {
                HttpRequest request_ = new HttpRequest();
                java.util.Map<String, String> systemParams = HttpConverter.buildMap(
                    new HttpPair("method", "alipay.user.certify.open.query"),
                    new HttpPair("app_id", _kernel.getConfig("appId")),
                    new HttpPair("timestamp", _kernel.getTimestamp()),
                    new HttpPair("format", "json"),
                    new HttpPair("version", "1.0"),
                ...
                java.util.Map<String, Object> bizParams = HttpConverter.buildMap(
                    new HttpPair("certify_id", certifyId)
                );
                java.util.Map<String, String> textParams = new java.util.HashMap<>();
                request_.protocol = _kernel.getConfig("protocol");
                request_.method = "POST";
                ...
                if (_kernel.isCertMode()) {
                    if (_kernel.verify(respMap, _kernel.extractAlipayPublicKey(_kernel.getAlipayCertSN(respMap)))) {
                        return HttpModel.toModel(_kernel.toRespModel(respMap), new AlipayUserCertifyOpenQueryResponse());
                    }
    ...
    public AlipayUserCertifyOpenCertifyResponse certify(String certifyId) throws Exception {
        java.util.Map<String, String> systemParams = HttpConverter.buildMap(
            new HttpPair("method", "alipay.user.certify.open.certify"),
            new HttpPair("app_id", _kernel.getConfig("appId")),
            new HttpPair("timestamp", _kernel.getTimestamp()),
            new HttpPair("format", "json"),
            new HttpPair("version", "1.0"),
        ...
        java.util.Map<String, String> response = HttpConverter.buildMap(
            new HttpPair("body", _kernel.generatePage("GET", systemParams, bizParams, textParams, sign))
        );
        return HttpModel.toModel(response, new AlipayUserCertifyOpenCertifyResponse());
    }
