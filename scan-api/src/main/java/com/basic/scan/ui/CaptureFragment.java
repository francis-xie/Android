...
import com.basic.scan.R;
import com.basic.scan.Scan;
import com.basic.scan.camera.CameraManager;
import com.basic.scan.decoding.CaptureViewHandler;
import com.basic.scan.decoding.InactivityTimer;
import com.basic.scan.util.QRCodeAnalyzeUtils;
import com.basic.scan.view.ViewfinderView;
...
public class CaptureFragment extends Fragment implements ICaptureView, SurfaceHolder.Callback {
    ...
    public static CaptureFragment newInstance(int layoutId, boolean isRepeated, long scanInterval) {
        ...
        bundle.putInt(Scan.KEY_LAYOUT_ID, layoutId);
        bundle.putBoolean(Scan.KEY_IS_REPEATED, isRepeated);
        bundle.putLong(Scan.KEY_SCAN_INTERVAL, scanInterval);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = null;
        if (bundle != null) {
            int layoutId = bundle.getInt(Scan.KEY_LAYOUT_ID);
            if (layoutId != -1) {
                view = inflater.inflate(layoutId, null);
            }
            mIsRepeated = bundle.getBoolean(Scan.KEY_IS_REPEATED);
            mScanInterval = bundle.getLong(Scan.KEY_SCAN_INTERVAL);
        }

        if (view == null) {
            view = inflater.inflate(R.layout.scan_fragment_capture, null);
        }
