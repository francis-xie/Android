...
import com.basic.page.core.CorePageManager;
import com.basic.page.core.CoreSwitchBean;
import com.basic.page.core.CoreSwitcher;
...
public class PageActivity extends AppCompatActivity implements CoreSwitcher {
    /**
     * 应用中所有PageActivity的引用
     */
    private static List<WeakReference<PageActivity>> sActivities = new ArrayList<>();
    /**
     * 当前activity的引用
     */
    private WeakReference<PageActivity> mCurrentActivity = null;
    /**
     ...
     * ForResult 的fragment
     */
    private PageFragment mFragmentForResult = null;
    /**
     * 请求码，必须大于等于0
     */
     ...
     * @return 栈顶Activity
     */
    public static PageActivity getTopActivity() {
        if (sActivities != null) {
            int size = sActivities.size();
            if (size >= 1) {
                WeakReference<PageActivity> ref = sActivities.get(size - 1);
                if (ref != null) {
                    return ref.get();
                }
            }
    ...
    protected String getPageName() {
        PageFragment frg = getActiveFragment();
        if (frg != null) {
     ...
     * 结束activity，设置是否显示动画
     *
     * @param activity      PageActivity对象
     * @param showAnimation 是否显示动画
     */
    private void finishActivity(PageActivity activity, boolean showAnimation) {
        if (activity != null) {
            activity.finish();
            //从activity列表中移除当前实例
    ...
    public boolean isFragmentTop(String fragmentTag) {
        int size = sActivities.size();
        if (size > 0) {
            WeakReference<PageActivity> reference = sActivities.get(size - 1);
            PageActivity activity = reference.get();
    ...
    public boolean findPage(String pageName) {
        ...
        for (int j = size - 1; j >= 0; j--) {
            WeakReference<PageActivity> reference = sActivities.get(j);
            if (reference != null) {
                PageActivity activity = reference.get();
                if (activity == null) {
    ...
    public Fragment gotoPage(CoreSwitchBean page) {
        ...
        for (int i = size - 1; i >= 0; i--) {
            WeakReference<PageActivity> ref = sActivities.get(i);
            if (ref != null) {
                PageActivity item = ref.get();
                if (item == null) {
                    PageLog.d("item null");
                    continue;
                }
     ...
     * @return 是否弹出成功
     */
    protected boolean popFragmentInActivity(final String pageName, Bundle bundle, PageActivity findActivity) {
        if (pageName == null || findActivity == null || findActivity.isFinishing()) {
            ...
            if (fragmentManager != null) {
                ...
                if (frg != null && frg instanceof PageFragment) {
                    ...
                    ((PageFragment) frg).onFragmentDataReset(bundle);
                    return true;
                }
            }
        }
     ...
     */
    @Override
    public Fragment openPageForResult(CoreSwitchBean page, PageFragment fragment) {
        if (page != null) {
            if (page.isNewActivity()) {
                ...
                PageFragment frg = CorePageManager.getInstance().openPageWithNewFragmentManager(getSupportFragmentManager(), pageName, bundle, animations, addToBackStack);
                if (frg == null) {
                    return null;
                }
                final PageFragment opener = fragment;
                frg.setRequestCode(page.getRequestCode());
                frg.setFragmentFinishListener(new PageFragment.OnFragmentFinishListener() {
                    @Override
     ...
     * @return 打开的fragment对象
     */
    public <T extends PageFragment> T openPage(Class<T> clazz) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends PageFragment> T openPage(Class<T> clazz, Bundle bundle) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends PageFragment> T changePage(Class<T> clazz) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends PageFragment> T changePage(Class<T> clazz, Bundle bundle) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        PageLog.d("onActivityResult from PageActivity" + requestCode + " " + resultCode);
    ...
    public PageFragment getActiveFragment() {
        ...
        if (manager != null) {
            ...
            if (count > 0) {
                ...
                return (PageFragment) manager.findFragmentByTag(tag);
            }
        }
        return null;
    }
    ...
    /**
     * 根据页面的类获取PageFragment
     *
     * @return 获取的页面
     */
    public <T extends PageFragment> T getPage(Class<T> clazz) {
        String pageName = PageConfig.getPageInfo(clazz).getName();
        PageFragment fragment = getPageByName(pageName);
        if (fragment != null) {
            return (T) fragment;
        } else {
            return null;
    ...
    /**
     * 根据页面的名称获取PageFragment
     *
     * @return 获取的页面
     */
    public PageFragment getPageByName(String pageName) {
        if (this.isFinishing() || TextUtils.isEmpty(pageName)) {
            return null;
        }
        FragmentManager manager = this.getSupportFragmentManager();
        if (manager != null) {
            return (PageFragment) manager.findFragmentByTag(pageName);
        }
        return null;
    }
    ...
    protected void printAllActivities() {
        PageLog.d("------------PageActivity print all------------activities size:" + sActivities.size());
        for (WeakReference<PageActivity> ref : sActivities) {
            if (ref != null) {
                PageActivity item = ref.get();
                if (item != null) {
            ...
            if (page != null) {
                ...
                if (fragment != null) {
                    if (startActivityForResult) {
                        ...
                        fragment.setFragmentFinishListener(new PageFragment.OnFragmentFinishListener() {
                            @Override
                            public void onFragmentResult(int requestCode, int resultCode, Intent intent) {
                                PageActivity.this.setResult(resultCode, intent);
                            }
                        });
                    }
                } else {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        PageFragment activeFragment = getActiveFragment();
