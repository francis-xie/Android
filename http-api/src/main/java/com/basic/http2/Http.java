...
import com.basic.http2.cache.converter.SerializableDiskConverter;
import com.basic.http2.cache.model.CacheMode;
import com.basic.http2.cookie.CookieManager;
import com.basic.http2.https.DefaultHostnameVerifier;
import com.basic.http2.https.HttpsUtils;
...
import com.basic.http2.model.HttpParams;
import com.basic.http2.request.CustomRequest;
import com.basic.http2.request.DeleteRequest;
import com.basic.http2.request.DownloadRequest;
import com.basic.http2.request.GetRequest;
import com.basic.http2.request.PostRequest;
import com.basic.http2.request.PutRequest;
import com.basic.http2.utils.RxSchedulers;
...
public final class Http {
    private volatile static Http sInstance = null;
     ...
     */
    public Http debug(HttpLoggingInterceptor loggingInterceptor) {
     ...
     */
    public Http debug(boolean isDebug) {
        if (isDebug) {
     ...
     */
    public Http setStrictMode(boolean strictMode) {
     ...
     * 全局设置subUrl
     */
    public Http setSubUrl(String subUrl) {
     ...
     * 全局设置超时重试延迟时间
     */
    public Http setRetryDelay(int retryDelay) {
        if (retryDelay < 0) {
     ...
     * 全局设置超时重试延迟叠加时间
     */
    public Http setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 设置全局的缓存模式
     */
    public Http setCacheMode(CacheMode cacheMode) {
     ...
     * @return
     */
    public Http setIsDiskCache(boolean isDiskCache) {
     ...
     * @return
     */
    public Http setMemoryMaxSize(int memoryMaxSize) {
     ...
     * 设置全局的缓存过期时间
     */
    public Http setCacheTime(long cacheTime) {
        if (cacheTime <= -1) {
     ...
     * 设置全局的缓存大小,默认50M
     */
    public Http setCacheMaxSize(long maxSize) {
     ...
     */
    public Http setCacheVersion(int cacheVersion) {
        if (cacheVersion < 0) {
     ...
     */
    public Http setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public Http setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存, 默认是3天
     */
    public Http setHttpCache(Cache cache) {
        mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public Http addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public Http addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局网络拦截器
     */
    public Http addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public Http setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public Http setOkconnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public Http setOkclient(OkHttpClient client) {
     ...
     */
    public Http addConverterFactory(Converter.Factory factory) {
     ...
     */
    public Http addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public Http setCallbackExecutor(Executor executor) {
     ...
     */
    public Http setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * https的全局自签名证书
     */
    public Http setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public Http setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public Http setCookieStore(CookieManager cookieManager) {
