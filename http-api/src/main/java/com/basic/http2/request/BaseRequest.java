...
import com.basic.http2.api.ApiService;
...
import com.basic.http2.callback.CallClazzProxy;
import com.basic.http2.https.HttpsUtils;
import com.basic.http2.interceptor.BaseDynamicInterceptor;
import com.basic.http2.interceptor.CacheInterceptor;
import com.basic.http2.interceptor.CacheInterceptorOffline;
import com.basic.http2.interceptor.HeadersInterceptor;
import com.basic.http2.interceptor.NoCacheInterceptor;
...
import com.basic.http2.transform.func.ApiResultFunc;
...
import static com.basic.http2.Http.DEFAULT_CACHE_NEVER_EXPIRE;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        mContext = Http.getContext();
        mUrl = url;
        mBaseUrl = Http.getBaseUrl();
        mSubUrl = Http.getSubUrl();
        if (!TextUtils.isEmpty(mBaseUrl)) {
            mHttpUrl = HttpUrl.parse(mBaseUrl);
        }
        // 添加缓存模式
        mCacheMode = Http.getCacheMode();
        // 缓存时间
        mCacheTime = Http.getCacheTime();
        // 超时重试次数
        mRetryCount = Http.getRetryCount();
        // 超时重试延时
        mRetryDelay = Http.getRetryDelay();
        // 超时重试叠加延时
        mRetryIncreaseDelay = Http.getRetryIncreaseDelay();
        // OKHttp  mCache
        mCache = Http.getHttpCache();
        ...
        if (Http.getCommonParams() != null) {
            mParams.put(Http.getCommonParams());
        }
        if (Http.getCommonHeaders() != null) {
            mHeaders.put(Http.getCommonHeaders());
        }
    }
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (mReadTimeOut <= 0 && mWriteTimeOut <= 0 && mConnectTimeout <= 0 && mSSLParams == null
            ...
            final OkHttpClient.Builder newClientBuilder = Http.getOkHttpClient().newBuilder();
            if (mReadTimeOut > 0) {
            ...
            if (mCookies.size() > 0) {
                Http.getCookieJar().addCookies(mCookies);
            }
    ...
    private Retrofit.Builder generateRetrofit() {
        if (mConverterFactories.isEmpty() && mAdapterFactories.isEmpty()) {
            return Http.getRetrofitBuilder().baseUrl(mBaseUrl);
        } else {
            ...
            if (!mConverterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<Converter.Factory> listConverterFactory = Http.getRetrofitBuilder().converterFactories();
            ...
            if (!mAdapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = Http.getRetrofitBuilder().callAdapterFactories();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = Http.getRxCacheBuilder();
        switch (mCacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用OkHttp的缓存
                if (mCache == null) {
                    File cacheDirectory = Http.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(Http.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    mCache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, Http.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, mCacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(Http.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(Http.getContext(), cacheControlValue);
            ...
            case CACHE_REMOTE_DISTINCT:
                ...
                if (mDiskConverter == null) {
                    ...
                    final RxCache.Builder cacheBuilder = Http.getRxCache().newBuilder();
