...
/**
 * 作为通用列表 {@link FACEGroupListView} 里的 item 使用，也可以单独使用。
 * 支持以下样式:
 * <ul>
 ...
 * 也可以在 xml 中使用 {@link R.styleable#FACECommonListItemView_face_orientation} 设置。</li>
...
public class FACECommonListItemView extends RelativeLayout {
    ...
    public @interface FACECommonListItemAccessoryType {
    }

    @IntDef({VERTICAL, HORIZONTAL})
    @Retention(RetentionPolicy.SOURCE)
    public @interface FACECommonListItemOrientation {
    }

    @IntDef({RED_DOT_POSITION_LEFT, RED_DOT_POSITION_RIGHT})
    @Retention(RetentionPolicy.SOURCE)
    public @interface FACECommonListItemRedDotPosition {
    }

    /**
     * Item 右侧的 View 的类型
     */
    @FACECommonListItemAccessoryType
     ...
     * 控制红点的位置
     */
    @FACECommonListItemRedDotPosition
    ...
    public FACECommonListItemView(Context context) {
        this(context, null);
    }

    public FACECommonListItemView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.FACECommonListItemViewStyle);
    }

    public FACECommonListItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        LayoutInflater.from(context).inflate(R.layout.face_layout_common_list_item, this, true);

        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.FACECommonListItemView, defStyleAttr, 0);
        @FACECommonListItemOrientation int orientation = array.getInt(R.styleable.FACECommonListItemView_face_orientation, HORIZONTAL);
        @FACECommonListItemAccessoryType int accessoryType = array.getInt(R.styleable.FACECommonListItemView_face_accessory_type, ACCESSORY_TYPE_NONE);
        final int initTitleColor = array.getColor(R.styleable.FACECommonListItemView_face_commonList_titleColor, ResUtils.getColor(R.color.face_config_color_gray_1));
        final int initDetailColor = array.getColor(R.styleable.FACECommonListItemView_face_commonList_detailColor, ResUtils.getColor(R.color.face_config_color_gray_5));
        array.recycle();
        ...
        if (ViewUtils.getIsLastLineSpacingExtraError()) {
            detailTextViewLP.bottomMargin = -ThemeUtils.resolveDimension(context, R.attr.face_common_list_item_detail_line_space);
        }
    ...
    public void setRedDotPosition(@FACECommonListItemRedDotPosition int redDotPosition) {
    ...
    public void setOrientation(@FACECommonListItemOrientation int orientation) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            spaceLp.weight = 0;
            mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ThemeUtils.resolveDimension(getContext(), R.attr.face_common_list_item_title_v_text_size));
            mDetailTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ThemeUtils.resolveDimension(getContext(), R.attr.face_common_list_item_detail_v_text_size));
        } else {
            ...
            spaceLp.height = 0;
            spaceLp.weight = 1;
            mTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ThemeUtils.resolveDimension(getContext(), R.attr.face_common_list_item_title_h_text_size));
            mDetailTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, ThemeUtils.resolveDimension(getContext(), R.attr.face_common_list_item_detail_h_text_size));
        }
    }
     ...
     * </p>
     *
     * @param type 见 {@link FACECommonListItemAccessoryType}
     */
    public void setAccessoryType(@FACECommonListItemAccessoryType int type) {
        ...
        switch (type) {
            ...
            case ACCESSORY_TYPE_CHEVRON: {
                ...
                Drawable chevronDrawable = ThemeUtils.resolveDrawable(getContext(), R.attr.face_common_list_item_chevron, ResUtils.getDrawable(getContext(), R.drawable.face_icon_chevron));
            ...
            case ACCESSORY_TYPE_SWITCH: {
                if (mSwitch == null) {
                    ...
                    mSwitch.setButtonDrawable(ThemeUtils.resolveDrawable(getContext(), R.attr.face_common_list_item_switch));
