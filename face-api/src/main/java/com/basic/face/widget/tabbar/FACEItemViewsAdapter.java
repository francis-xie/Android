 ...
 * 复用已存在的 {@link View}。
 * FACE 用于 {@link TabSegment} 中 {@link TabSegment.Tab} 与数据的适配。
 */
public abstract class FACEItemViewsAdapter<T, V extends View> {
    ...
    public FACEItemViewsAdapter(ViewGroup parentView) {
    ...
    public void detach(int count) {
        ...
        while (childCount > 0 && count > 0) {
            ...
            Object notCacheTag = view.getTag(R.id.face_view_can_not_cache_tag);
    ...
    public FACEItemViewsAdapter<T, V> addItem(T item) {
        mItemData.add(item);
        return this;
    }
