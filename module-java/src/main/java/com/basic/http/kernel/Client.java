...
import com.basic.http.kernel.util.*;
...
public class Client {
    ...
    public byte[] toUrlEncodedRequestBody(Map<String, ?> bizParams) throws Exception {
        ...
        return buildQueryString(sortedMap).getBytes(Constants.DEFAULT_CHARSET);
    }
    ...
    public Map<String, Object> readAsJson(HttpResponse response, String method) throws Exception {
        ...
        map.put(Constants.BODY_FIELD, responseBody);
        map.put(Constants.METHOD_FIELD, method);
    ...
    public Map<String, Object> toRespModel(Map<String, Object> respMap) throws Exception {
        String methodName = (String) respMap.get(Constants.METHOD_FIELD);
        String responseNodeName = methodName.replace('.', '_') + Constants.RESPONSE_SUFFIX;
        String errorNodeName = Constants.ERROR_RESPONSE;
        ...
        for (Entry<String, Object> pair : respMap.entrySet()) {
            if (responseNodeName.equals(pair.getKey())) {
                ...
                model.put(Constants.BODY_FIELD, respMap.get(Constants.BODY_FIELD));
                return model;
            }
        }
     ...
     */
    public java.io.InputStream toMultipartRequestBody(Map<String, String> textParams,
                                                      Map<String, String> fileParams, String boundary) throws Exception {
     ...
     * @return 生成的URL字符串或表单
     */
    public String generatePage(String method, Map<String, String> systemParams, Map<String, ?> bizParams,
        ...
        if (Constants.GET.equalsIgnoreCase(method)) {
            ...
            sortedMap.put(Constants.SIGN_FIELD, sign);
            ...
            urlParams.put(Constants.SIGN_FIELD, sign);
            ...
            formParams.put(Constants.BIZ_CONTENT_FIELD, JsonUtil.toJsonString(bizParams));
     ...
     * @return 支付宝公钥证书序列号
     */
    public String getCertSN(Map<String, Object> respMap) throws Exception {
        return (String) respMap.get(Constants.ALIPAY_CERT_SN_FIELD);
    }
     ...
     * @return 支付宝根证书序列号
     */
    public String getRootCertSN() throws Exception {
        if (context.getCertEnvironment() == null) {
            return null;
        }
     ...
     * @return 支付宝公钥
     */
    public String extractPublicKey(String alipayCertSN) throws Exception {
        if (context.getCertEnvironment() == null) {
            return null;
        }
        return context.getCertEnvironment().getPublicKey(alipayCertSN);
    }
    ...
    public Boolean verify(Map<String, Object> respMap, String alipayPublicKey) throws Exception {
        String sign = (String) respMap.get(Constants.SIGN_FIELD);
        String content = SignContentExtractor.getSignSourceData((String) respMap.get(Constants.BODY_FIELD),
                (String) respMap.get(Constants.METHOD_FIELD));
        return Signer.rsaCheckContent(content, sign, alipayPublicKey, "UTF-8");
     ...
     * @return 签名值的Base64串
     */
    public String sign(Map<String, String> systemParams, Map<String, ?> bizParams,
                       Map<String, String> textParams, String merchantPrivateKey) throws Exception {
        Map<String, String> sortedMap = getSortedMap(systemParams, bizParams, textParams);
        ...
        return context.getSigner().rsaSign(content.toString(), merchantPrivateKey, "UTF-8");
...
//        return context.getSigner().sign(content.toString(), merchantPrivateKey);
     ...
     * @return 订单串
     */
    public String generateOrderString(Map<String, String> systemParams, Map<String, Object> bizParams,
                                      Map<String, String> textParams, String sign) throws Exception {
        //采集并排序所有参数
        Map<String, String> sortedMap = getSortedMap(systemParams, bizParams, textParams);
        sortedMap.put(Constants.SIGN_FIELD, sign);
    ...
    private Map<String, String> getSortedMap(Map<String, String> systemParams, Map<String, ?> bizParams,
        ...
        if (bizParams != null && !bizParams.isEmpty()) {
            sortedMap.put(Constants.BIZ_CONTENT_FIELD, JsonUtil.toJsonString(bizParams));
        }
        if (textParams != null) {
    ...
    private void setNotifyUrl(Map<String, String> params) throws Exception {
        if (getConfig(Constants.NOTIFY_URL_CONFIG_KEY) != null && !params.containsKey(Constants.NOTIFY_URL_FIELD)) {
            params.put(Constants.NOTIFY_URL_FIELD, getConfig(Constants.NOTIFY_URL_CONFIG_KEY));
        }
    }
        ...
        for (Entry<String, String> pair : sortedMap.entrySet()) {
            if (!Strings.isNullOrEmpty(pair.getKey()) && !Strings.isNullOrEmpty(pair.getValue())) {
                content.append(index == 0 ? "" : "&")
                        .append(pair.getKey())
                        .append("=")
                        .append(URLEncoder.encode(pair.getValue(), Constants.DEFAULT_CHARSET.name()));
                index++;
            }
        }
    ...
    private String getGatewayServerUrl() throws Exception {
        return getConfig(Constants.PROTOCOL_CONFIG_KEY) + "://" + getConfig(Constants.HOST_CONFIG_KEY) + "/gateway.do";
    }
}
