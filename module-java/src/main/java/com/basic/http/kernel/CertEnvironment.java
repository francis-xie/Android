...
import com.basic.http.kernel.util.AntCertificationUtil;
...
public class CertEnvironment {
     ...
     */
    private Map<String, String> cachedPublicKey = new ConcurrentHashMap<String, String>();
    ...
    public CertEnvironment(String merchantCertPath, String alipayCertPath, String alipayRootCertPath) {
        ...
        cachedPublicKey.put(AntCertificationUtil.getCertSN(alipayPublicCertContent),
    ...
    public String getPublicKey(String sn) {
        //如果没有指定sn，则默认取缓存中的第一个值
        if (Strings.isNullOrEmpty(sn)) {
            return cachedPublicKey.values().iterator().next();
        }

        if (cachedPublicKey.containsKey(sn)) {
            return cachedPublicKey.get(sn);
        } else {
