...
package com.basic.http.util.generic;

import com.basic.http.*;
import com.basic.http.util.generic.models.OpenApiGenericResponse;
import com.basic.http.util.generic.models.OpenApiGenericSDKResponse;
...
public class Client {
    ...
    public OpenApiGenericResponse execute(String method, java.util.Map<String, String> textParams, java.util.Map<String, ?> bizParams) throws Exception {
            ...
            try {
                ...
                if (_kernel.isCertMode()) {
                    if (_kernel.verify(respMap, _kernel.extractPublicKey(_kernel.getCertSN(respMap)))) {
                        return HttpModel.toModel(_kernel.toRespModel(respMap), new OpenApiGenericResponse());
                    }
    ...
    public OpenApiGenericResponse fileExecute(String method, java.util.Map<String, String> textParams, java.util.Map<String, ?> bizParams, java.util.Map<String, String> fileParams) throws Exception {
        java.util.Map<String, Object> runtime_ = HttpConverter.buildMap(
            new HttpPair("ignoreSSL", _kernel.getConfig("ignoreSSL")),
            new HttpPair("httpProxy", _kernel.getConfig("httpProxy")),
            new HttpPair("connectTimeout", 100000),
            new HttpPair("readTimeout", 100000),
            new HttpPair("retry", HttpConverter.buildMap(
                new HttpPair("maxAttempts", 0)
            ))
        );
        ...
        while (Http.allowRetry((java.util.Map<String, Object>) runtime_.get("retry"), _retryTimes, _now)) {
            ...
            try {
                ...
                request_.headers = HttpConverter.buildMap(
                    new HttpPair("host", _kernel.getConfig("gatewayHost")),
                    new HttpPair("content-type", _kernel.concatStr("multipart/form-data;charset=utf-8;boundary=", boundary))
                );
                request_.query = _kernel.sortMap(HttpConverter.merge(String.class,
                    HttpConverter.buildMap(
                        new HttpPair("sign", _kernel.sign(systemParams, bizParams, textParams, _kernel.getConfig("merchantPrivateKey")))
    ...
    public OpenApiGenericSDKResponse sdkExecute(String method, java.util.Map<String, String> textParams, java.util.Map<String, Object> bizParams) throws Exception {
        HttpRequest request_ = new HttpRequest();
        java.util.Map<String, String> systemParams = HttpConverter.buildMap(
            new HttpPair("method", method),
            new HttpPair("app_id", _kernel.getConfig("appId")),
        ...
        java.util.Map<String, String> response = HttpConverter.buildMap(
            new HttpPair("body", _kernel.generateOrderString(systemParams, bizParams, textParams, sign))
        );
        return HttpModel.toModel(response    , new OpenApiGenericSDKResponse());
    }
