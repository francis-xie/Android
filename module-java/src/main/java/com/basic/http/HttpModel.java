...
import com.basic.http.logging.ClientLogger;
...
public class HttpModel {
    private static final ClientLogger logger = new ClientLogger(HttpModel.class);
    ...
    private static Map<String, Object> toMap(Object object, Boolean exceptStream) {
        ...
        if (null == object || !HttpModel.class.isAssignableFrom(object.getClass())) {
            return map;
        }
    ...
    private static Map<String, Object> changeToMap(Object object, Boolean exceptStream) {
        ...
        try {
            for (Field field : object.getClass().getFields()) {
                ...
                } else if (null != field.get(object) && HttpModel.class.isAssignableFrom(field.get(object).getClass())) {
                    map.put(key, HttpModel.toMap(field.get(object), exceptStream));
    ...
    public static Object parseObject(Object o) {
        ...
        if (List.class.isAssignableFrom(clazz)) {
            ...
            return result;
        } else if (HttpModel.class.isAssignableFrom(clazz)) {
            return ((HttpModel) o).toMap(false);
        } else {
            return o;
        }
    }
    ...
    private static Object buildObject(Object o, Class self, Type subType) {
        ...
        if (Map.class.isAssignableFrom(self) && Map.class.isAssignableFrom(valueClass)) {
            ...
            return result;
        } else if (HttpModel.class.isAssignableFrom(self) && Map.class.isAssignableFrom(valueClass)) {
            try {
                return HttpModel.toModel((Map<String, Object>) o, (HttpModel) self.newInstance());
            } catch (Exception e) {
                throw new HttpException(e.getMessage(), e);
            }
        } else {
    ...
    public static <T extends HttpModel> T toModel(Map<String, ?> map, T model) {
        T result = model;
        for (Field field : result.getClass().getFields()) {
            ...
            result = setHttpModelField(result, field, value, false);
        }
        return result;
    }

    @SuppressWarnings("unchecked")
    private static <T extends HttpModel> T setHttpModelField(T model, Field field, Object value, boolean userBuild) {
        try {
            Class<?> clazz = field.getType();
            Object resultValue = parseNumber(value, clazz);
            T result = model;
            if (HttpModel.class.isAssignableFrom(clazz)) {
                Object data = clazz.getDeclaredConstructor().newInstance();
                if (userBuild) {
                    field.set(result, HttpModel.build(HttpModel.toMap(resultValue, false), (HttpModel) data));
                } else if (!userBuild && Map.class.isAssignableFrom(resultValue.getClass())) {
                    field.set(result, HttpModel.toModel((Map<String, Object>) resultValue, (HttpModel) data));
                } else {
    ...
    @SuppressWarnings("unchecked")
    public static <T extends HttpModel> T build(Map<String, ?> map, T model) {
        T result = model;
        for (Field field : model.getClass().getFields()) {
            ...
            result = setHttpModelField(result, field, value, true);
        }
        return result;
    }
    ...
    public static Map<String, Object> buildMap(HttpModel httpModel) {
        if (null == httpModel) {
            return null;
        } else {
            return httpModel.toMap();
        }
    }

    public static void validateParams(HttpModel httpModel, String paramName) {
        if (null == httpModel) {
