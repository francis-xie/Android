...
import com.basic.tools.system.ThreadPoolManager;
import com.basic.tools.tip.ToastUtils;
import com.basic.tools_sub.R;
        ...
        public void run() {
            ...
            if (state == WifiManager.WIFI_STATE_ENABLED) {
                ...
                mWifiHandler.post(new Runnable() {
                    ...
                    public void run() {
                        startWifiApTh();
                        ToastUtils.toast(R.string.util_tip_close_wifi_success);
                    }
                });
            }
        }
        ...
        public void run() {
            ...
            if (state == WifiAPUtils.WIFI_AP_STATE_FAILED) {
                mWifiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.toast(R.string.util_tip_open_wifiap_failed);
                        if (mListener != null) {
                ...
                mWifiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.toast(R.string.util_tip_open_wifiap_success);
                        if (mListener != null) {
        ...
        public void run() {
            ...
            if (state == WifiAPUtils.WIFI_AP_STATE_ENABLED) {
                ...
                mWifiHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.toast(R.string.util_tip_close_wifiap_success);
                        if (mListener != null) {
                            mListener.onWifiAPStatusChanged(false);
                        }
                    }
