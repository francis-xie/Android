...
package com.basic.code.fragment.components.picker;
...
import com.basic.face.widget.picker.widget.TimePickerView;
import com.basic.face.widget.picker.widget.builder.OptionsPickerBuilder;
import com.basic.face.widget.picker.widget.builder.TimePickerBuilder;
import com.basic.face.widget.picker.widget.configure.TimePickerType;
...
public class TimePickerFragment extends BaseFragment {
    ...
    private void showDatePicker() {
        if (mDatePicker == null) {
            mDatePicker = new TimePickerBuilder(getContext(), (date, v) -> ToastUtils.toast(DateUtils.date2String(date, DateUtils.yyyyMMdd.get())))
...
//            mDatePicker.getWheelTime().getView().setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
    ...
    private void showTimePicker() {
        if (mTimePicker == null) {
            ...
            mTimePicker = new TimePickerBuilder(getContext(), (date, v) -> ToastUtils.toast(DateUtils.date2String(date, DateUtils.HHmmss.get())))
                    .setTimeSelectChangeListener(date -> Log.i("pvTime", "onTimeSelectChanged"))
    ...
    private void showTimePickerDialog() {
        if (mTimePickerDialog == null) {
            ...
            mTimePickerDialog = new TimePickerBuilder(getContext(), (date, v) -> ToastUtils.toast(DateUtils.date2String(date, DateUtils.yyyyMMddHHmmss.get())))
                    .setTimeSelectChangeListener(date -> Log.i("pvTime", "onTimeSelectChanged"))
    ...
    private void showTimePeriodPicker() {
        ...
        OptionsPickerView pvOptions = new OptionsPickerBuilder(getContext(), (v, options1, options2, options3) -> {
            ToastUtils.toast(mTimeOption[options1]);
            return false;
        })
                .setTitleText("时间点选择")
    ...
    private void showTimePeriodPicker1() {
        ...
        OptionsPickerView pvOptions = new OptionsPickerBuilder(getContext(), (v, options1, options2, options3) -> {
            ToastUtils.toast(option[options1] + mTimeOption1[options1][options2]);
            return false;
        })
                .setTitleText("时间点选择")
                .isRestoreItem(true)
    ...
    private void showTimePeriodPicker2() {
        ...
        OptionsPickerView pvOptions = new OptionsPickerBuilder(getContext(), (v, options1, options2, options3) -> {
            if (options1 > options2) {
                ...
                ToastUtils.toast(mTimeOption[options1] + "~" + mTimeOption[options2]);
                return false;
            }
        })
                .setTitleText("时间段选择")
