...
public class CrashActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTheme(getIntent().getIntExtra(KEY_THEME_ID, R.style.CrashTheme_Light));
        ...
        setContentView(R.layout.log_activity_crash);
            ...
            public void onClick(View v) {
                ...
                menu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        ...
                        if (id == R.id.log_crash_menu_copy_text) {
                            String crashText = getShareText(mCrashInfo);
                            ClipboardManager cm = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
                            if (cm != null) {
                                ClipData clipData = ClipData.newPlainText("crash", crashText);
                                cm.setPrimaryClip(clipData);
                                showToast(getString(R.string.log_crash_copied));
                            }
                        } else if (id == R.id.log_crash_menu_share_text) {
                            String crashText = getShareText(mCrashInfo);
                            shareText(crashText);
                        } else if (id == R.id.log_crash_menu_share_image) {
                        ...
                        } else if (id == R.id.log_crash_menu_send_email) {
                            if (ContextCompat.checkSelfPermission(CrashActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED
                                    || ContextCompat.checkSelfPermission(CrashActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED) {
    ...
    private String getShareText(CrashInfo model) {
        ...
        builder.append(getString(R.string.log_crash_crash_info))
                .append("\n")
        ...
        builder.append(getString(R.string.log_crash_packet_name))
                .append(model.getPackageName()).append("\n");
        builder.append(getString(R.string.log_crash_class_name))
                .append(model.getFileName()).append("\n");
        builder.append(getString(R.string.log_crash_method_name)).append(model.getMethodName()).append("\n");
        builder.append(getString(R.string.log_crash_line_number)).append(model.getLineNumber()).append("\n");
        builder.append(getString(R.string.log_crash_exception_type)).append(model.getExceptionType()).append("\n");
        builder.append(getString(R.string.log_crash_time)).append(getFormatDate(model)).append("\n");

        //空一行，好看点，(#^.^#)
        builder.append("\n");

        builder.append(getString(R.string.log_crash_model)).append(model.getDevice().getModel()).append("\n");
        builder.append(getString(R.string.log_crash_brand)).append(model.getDevice().getBrand()).append("\n");
        builder.append(getString(R.string.log_crash_version)).append(model.getDevice().getVersion()).append("\n");

        //空一行，好看点，(#^.^#)
        builder.append("\n");

        builder.append(getString(R.string.log_crash_all_info))
                .append("\n")
    ...
    private void shareText(String text) {
        ...
        intent.putExtra(Intent.EXTRA_SUBJECT, getString(R.string.log_crash_crash_info));
        intent.putExtra(Intent.EXTRA_TEXT, text);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        startActivity(Intent.createChooser(intent, getString(R.string.log_crash_share_to)));
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        if (requestCode == REQUEST_CODE_SHARE_IMAGE || requestCode == REQUEST_CODE_SEND_EMAIL) {
            ...
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                //授权失败
                showToast(R.string.log_crash_permission_sd);
            }
        } else {
    ...
    private File BitmapToFile(Bitmap bitmap) {
        ...
        String path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
                .getAbsolutePath();
        File imageFile = new File(path, "Log_" + getFormatDate(mCrashInfo) + ".jpg");
        try {
    ...
    private void shareImage() {
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            showToast(R.string.log_crash_no_sdcard);
            return;
        }
        File file = BitmapToFile(getBitmapByView(mToolbar, mScrollView));
        if (file == null || !file.exists()) {
            showToast(R.string.log_crash_image_not_exist);
            return;
        }
    ...
    private void sendEmail() {
        final MailInfo sender = mMailInfo
                .setTitle(getString(R.string.log_title_crash_report_email) + "【" + TimeUtils.getCurTime("yyyy-MM-dd HH:mm:ss") + "】")
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    sender.send();
                    showToast(R.string.log_email_send_success);
                } catch (Exception e) {
                    showToast(R.string.log_email_send_failed);
                    e.printStackTrace();
                }
            }
        }).start();
