...
public class FACEAlphaViewHelper implements IAlphaViewHelper {
    ...
    public FACEAlphaViewHelper(@NonNull View target) {
        mTarget = new WeakReference<>(target);
        mChangeAlphaWhenPress = ThemeUtils.resolveBoolean(target.getContext(), R.attr.face_switch_alpha_pressed, true);
        mChangeAlphaWhenDisable = ThemeUtils.resolveBoolean(target.getContext(), R.attr.face_switch_alpha_disabled, true);
        mPressedAlpha = ThemeUtils.resolveFloat(target.getContext(), R.attr.face_alpha_pressed, 0.5F);
        mDisabledAlpha = ThemeUtils.resolveFloat(target.getContext(), R.attr.face_alpha_disabled, 0.5F);
    }

    public FACEAlphaViewHelper(@NonNull View target, float pressedAlpha, float disabledAlpha) {
        mTarget = new WeakReference<>(target);
