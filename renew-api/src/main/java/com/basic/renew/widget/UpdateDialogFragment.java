...
import com.basic.renew.utils.ColorUtils;
import com.basic.renew.utils.DialogUtils;
import com.basic.renew.utils.DrawableUtils;
import com.basic.renew.utils.UpdateUtils;

import java.io.File;

import static com.basic.renew.entity.UpdateError.ERROR.DOWNLOAD_PERMISSION_DENIED;
import static com.basic.renew.entity.UpdateError.ERROR.PROMPT_UNKNOWN;
...
public class UpdateDialogFragment extends DialogFragment implements View.OnClickListener, IDownloadEventHandler {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        _Update.setIsPrompterShow(getUrl(), true);
        setStyle(DialogFragment.STYLE_NO_TITLE, R.style.Update_Fragment_Dialog);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.update_layout_update_prompter, container);
    }
    ...
    private void initUpdateInfo(UpdateEntity updateEntity) {
        ...
        mTvTitle.setText(String.format(getString(R.string.update_lab_ready_update), newVersion));
    ...
    private void initTheme(@ColorInt int themeColor, @DrawableRes int topResId, @ColorInt int buttonTextColor) {
        if (themeColor == -1) {
            themeColor = ColorUtils.getColor(getContext(), R.color.update_default_theme_color);
        }
        if (topResId == -1) {
            topResId = R.drawable.update_bg_app_top;
        }
    ...
    private void setDialogTheme(int themeColor, int topResId, int buttonTextColor) {
        Drawable topDrawable = _Update.getTopDrawable(mPromptEntity.getTopDrawableTag());
        if (topDrawable != null) {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_CODE_REQUEST_PERMISSIONS) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                _Update.onUpdateError(DOWNLOAD_PERMISSION_DENIED);
                dismissDialog();
            }
        }
    ...
    private void showInstallButton() {
        ...
        mBtnBackgroundUpdate.setVisibility(View.GONE);
        mBtnUpdate.setText(R.string.update_lab_install);
    ...
    private void showUpdateButton() {
        mNumberProgressBar.setVisibility(View.GONE);
        mBtnBackgroundUpdate.setVisibility(View.GONE);
        mBtnUpdate.setText(R.string.update_lab_update);
        mBtnUpdate.setVisibility(View.VISIBLE);
        mBtnUpdate.setOnClickListener(this);
    }

    private void onInstallApk() {
        _Update.startInstallApk(getContext(), UpdateUtils.getApkFileByUpdateEntity(mUpdateEntity), mUpdateEntity.getDownLoadEntity());
    }
    ...
    public void show(@NonNull FragmentManager manager, @Nullable String tag) {
        ...
        try {
            ...
            _Update.onUpdateError(PROMPT_UNKNOWN, e.getMessage());
        }
    }
    ...
    private void reloadView() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.update_layout_update_prompter, null);
