...
import static com.basic.renew.entity.UpdateError.ERROR.CHECK_NO_NETWORK;
import static com.basic.renew.entity.UpdateError.ERROR.CHECK_NO_WIFI;
import static com.basic.renew.entity.UpdateError.ERROR.PROMPT_ACTIVITY_DESTROY;
...
import com.basic.renew.proxy.impl.DefaultUpdatePrompter;
...
public class UpdateManager implements IUpdateProxy {
    ...
    @Override
    public void update() {
        UpdateLog.d("Update.update()启动:" + this);
    ...
    private void doCheck() {
        if (mIsWifiOnly) {
            if (UpdateUtils.checkWifi()) {
                ...
                onAfterCheck();
                _Update.onUpdateError(CHECK_NO_WIFI);
            }
        } else {
            if (UpdateUtils.checkNetwork()) {
                ...
                onAfterCheck();
                _Update.onUpdateError(CHECK_NO_NETWORK);
            }
        }
    }
            ...
            if (mUpdateProxy != null) {
                ...
                if (mIUpdatePrompter instanceof DefaultUpdatePrompter) {
                    ...
                    if (context instanceof FragmentActivity && ((FragmentActivity) context).isFinishing()) {
                        _Update.onUpdateError(PROMPT_ACTIVITY_DESTROY);
                    } else {
    ...
    public static class Builder {
        ...
        Builder(@NonNull Context context) {
            ...
            params = new TreeMap<>();
            if (_Update.getParams() != null) {
                params.putAll(_Update.getParams());
            }

            promptEntity = new PromptEntity();

            updateHttpService = _Update.getIUpdateHttpService();

            updateChecker = _Update.getIUpdateChecker();
            updateParser = _Update.getIUpdateParser();
            updatePrompter = _Update.getIUpdatePrompter();
            updateDownLoader = _Update.getIUpdateDownLoader();

            isGet = _Update.isGet();
            isWifiOnly = _Update.isWifiOnly();
            isAutoMode = _Update.isAutoMode();
            apkCacheDir = _Update.getApkCacheDir();
        }
        ...
        public Builder promptTopDrawable(Drawable topDrawable) {
            if (topDrawable != null) {
                String tag = _Update.saveTopDrawable(topDrawable);
        ...
        public Builder promptTopBitmap(Bitmap topBitmap) {
            if (topBitmap != null) {
                String tag = _Update.saveTopDrawable(new BitmapDrawable(context.getResources(), topBitmap));
                promptEntity.setTopDrawableTag(tag);
            }
            return this;
        }
