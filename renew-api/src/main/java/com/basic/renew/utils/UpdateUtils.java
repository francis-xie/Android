...
import static com.basic.renew.entity.UpdateError.ERROR.CHECK_APK_CACHE_DIR_EMPTY;
import static com.basic.renew.entity.UpdateError.ERROR.CHECK_IGNORED_VERSION;
...
public final class UpdateUtils {
    ...
    private static final String PREFS_FILE = "update_prefs";
        ...
        if (updateEntity != null) {
            if (updateEntity.isHasUpdate()) {
                ...
                if (updateEntity.isIgnorable() && UpdateUtils.isIgnoreVersion(updateProxy.getContext(), updateEntity.getVersionName())) {
                    _Update.onUpdateError(CHECK_IGNORED_VERSION);
                    //校验apk下载缓存目录是否为空
                } else if (TextUtils.isEmpty(updateEntity.getApkCacheDir())) {
                    _Update.onUpdateError(CHECK_APK_CACHE_DIR_EMPTY);
                } else {
            ...
            _Update.onUpdateError(CHECK_PARSE, "json:" + result);
        }
    }
    ...
    public static boolean isApkDownloaded(UpdateEntity updateEntity) {
        ...
        return !TextUtils.isEmpty(updateEntity.getMd5())
                ...
                && _Update.isFileValid(updateEntity.getMd5(), appFile);
    }
    ...
    public static String getDefaultDiskCacheDirPath() {
        return UpdateUtils.getDiskCacheDir(Update.getContext(), KEY_UPDATE);
    }
    ...
    public static boolean startActivity(final Intent intent) {
        ...
        if (Update.getContext().getPackageManager().resolveActivity(intent, PackageManager.MATCH_DEFAULT_ONLY) != null) {
            try {
